...
export class ColliderDebug {
  ...
  enable() {
    ...
    console.log('[ColliderDebug] 🟡 ON — amarelo=ellipsoid(real), vermelho=corpo físico, verde=estático');
  }
  ...
  _update() {
    ...
    for (const m of this.scene.meshes) {
      ...
      const isPhysBody = m.name.includes('_col') && m.isVisible === false;
      const pos = m.getAbsolutePosition();

      // ── Ellipsoid (forma REAL de colisão) — amarelo ────────────────
      const e = m.ellipsoid;
      if (e) {
        if (!h.ell) {
      ...
      h.box.material = this._mat(isPhysBody ? 'phys' : 'static', isPhysBody ? [1, 0.2, 0.2] : [0.2, 1, 0.3]);
